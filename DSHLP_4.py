...
def obj_rule_4(m):
    # Sum of Profit for all timeperiods
    return sum(m.nb[t].s * P4 - m.nb[t].r * C4 - m.nb[t].i * IC4 - m.nb[t].bl * BC4 -
               m.rho/2 * (m.nb[t].r - m.z34_1[t] + m.u34_1[t])**2
               if (t - m.delay < m.T.first() or m.TimeStep + m.delay > m.NumPeriods - 1) else
               m.nb[t].s * P4 - m.nb[t].r * C4 - m.nb[t].i * IC4 - m.nb[t].bl * BC4 -
               m.rho / 2 * (m.nb[t].r - m.z34_1[t] + m.u34_1[t]) ** 2 -
               m.rho / 2 * (m.nb[t].a - m.z34_2[t] + m.u34_2[t]) ** 2
               for t in m.T)
...
for j in range(num_tests):
    ...
    for d in range(num_periods):
        ...
        for i in range(N_iter):
            ...
            old_z = np.concatenate((old_z12_1, old_z23_1, old_z34_1))
            z = np.concatenate((z12_1_arr, z23_1_arr, z34_1_arr))
