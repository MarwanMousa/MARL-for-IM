...
path = 'LP_results/div_2/Oracle_DSHLP/'
failed_tests = np.load("LP_results/div_2/DSHLP/failed_tests.npy", allow_pickle=True)
num_tests = 200
test_seed = 420
...
for j in range(num_tests):
    ...
    BC5 = backlog_cost[4]  # shortage cost 5
    BC6 = backlog_cost[5]  # shortage cost 6
    ...
    I5 = inv_max[4]  # maximum inventory 5
    I6 = inv_max[5]  # maximum inventory 6

    O1 = inv_max[0]  # maximum order 1
    O2 = inv_max[1]  # maximum order 2
    O3 = inv_max[2]  # maximum order 3
    O4 = inv_max[3]  # maximum order 4
    O5 = inv_max[4]  # maximum order 5
    O6 = inv_max[5]  # maximum order 6
    ...
    def lotsizing_block_rule(b, t):
        ...
        b.backlog4 = pyo.Constraint(expr=b.bl4 == b.bl40 - b.s4 + d[t][0])
        b.backlog5 = pyo.Constraint(expr=b.bl5 == b.bl50 - b.s5 + d[t][1])
        b.backlog6 = pyo.Constraint(expr=b.bl6 == b.bl60 - b.s6 + d[t][2])
        ...
        b.ship4_bl = pyo.Constraint(expr=b.s4 <= b.bl40 + d[t][0])

        b.ship5_inv = pyo.Constraint(expr=b.s5 <= b.i50 + b.a5)
        b.ship5_bl = pyo.Constraint(expr=b.s5 <= b.bl50 + d[t][1])

        b.ship6_inv = pyo.Constraint(expr=b.s6 <= b.i60 + b.a6)
        b.ship6_bl = pyo.Constraint(expr=b.s6 <= b.bl60 + d[t][2])
    ...
    if j == 0:
        array_obs[:, :, 0] = s
    while not done:
        lp_action = [pyo.value(model.lsb[t].x1), pyo.value(model.lsb[t].x2), pyo.value(model.lsb[t].x3),
                     pyo.value(model.lsb[t].x4), pyo.value(model.lsb[t].x5), pyo.value(model.lsb[t].x6)]
