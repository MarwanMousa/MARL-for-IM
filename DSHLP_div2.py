...
def node_div_block_rule(b, t):
    ...
    # backlog
    b.bl1 = pyo.Var(domain=pyo.NonNegativeReals)
    b.bl2 = pyo.Var(domain=pyo.NonNegativeReals)
    b.bl = pyo.Var(domain=pyo.NonNegativeReals)

    # Initial Backlog at each time-step
    b.bl10 = pyo.Var(domain=pyo.NonNegativeReals)
    b.bl20 = pyo.Var(domain=pyo.NonNegativeReals)
    b.bl0 = pyo.Var(domain=pyo.NonNegativeReals)

    # Shipped goods/sales
    b.s1 = pyo.Var(domain=pyo.NonNegativeReals)
    b.s2 = pyo.Var(domain=pyo.NonNegativeReals)
    b.s = pyo.Var(domain=pyo.NonNegativeReals)
    ...
    # Customer demand
    b.demand1 = pyo.Var(domain=pyo.NonNegativeReals)
    b.demand2 = pyo.Var(domain=pyo.NonNegativeReals)
    b.demand = pyo.Var(domain=pyo.NonNegativeReals)
    ...
    # backlog constrains
    b.backlog1 = pyo.Constraint(expr=b.bl1 == b.bl10 - b.s1 + b.demand1)
    b.backlog2 = pyo.Constraint(expr=b.bl2 == b.bl20 - b.s2 + b.demand2)
    b.backlog = pyo.Constraint(expr=b.bl == b.bl1 + b.bl2)
    b.backlog0 = pyo.Constraint(expr=b.bl0 == b.bl10 + b.bl20)

    # Sales Constraints
    b.ship_sum = pyo.Constraint(expr=b.s == b.s1 + b.s2)
    b.ship_inventory = pyo.Constraint(expr=b.s <= b.i0 + b.a)
    b.ship_backlog1 = pyo.Constraint(expr=b.s1 <= b.bl10 + b.demand1)
    b.ship_backlog2 = pyo.Constraint(expr=b.s2 <= b.bl20 + b.demand2)
...
def bl1_linking_rule(m, t):
    if t == m.T.first():
        return m.nb[t].bl10 == m.B10
    return m.nb[t].bl10 == m.nb[t-1].bl1


def bl2_linking_rule(m, t):
    if t == m.T.first():
        return m.nb[t].bl20 == m.B20
    return m.nb[t].bl20 == m.nb[t-1].bl2
...
def obj_rule_2(m):
    ...
    for t in m.T:
        ...
        obj -= m.rho/2 * (m.nb[t].demand1 - m.z23_1[t] + m.u23_1[t])**2  # constraint 23 orders
        obj -= m.rho/2 * (m.nb[t].demand2 - m.z24_1[t] + m.u24_1[t])**2  # constraint 24 orders
        ...
        if t - m.delay3 >= m.T.first() and m.TimeStep + m.delay3 <= m.NumPeriods - 1:
            obj -= m.rho / 2 * (m.nb[t - m.delay3].s1 - m.z23_2[t] + m.u23_2[t]) ** 2  # constraint 23 goods
        if t - m.delay4 >= m.T.first() and m.TimeStep + m.delay4 <= m.NumPeriods - 1:
            obj -= m.rho / 2 * (m.nb[t - m.delay4].s2 - m.z24_2[t] + m.u24_2[t]) ** 2  # constraint 14 goods
...
def obj_rule_3(m):
    ...
    for t in m.T:
        ...
        obj -= m.rho/2 * (m.nb[t].r - m.z23_1[t] + m.u23_1[t])**2  # constraint 12 orders
        obj -= m.rho/2 * (m.nb[t].demand1 - m.z35_1[t] + m.u35_1[t])**2  # constraint 23 orders
        obj -= m.rho/2 * (m.nb[t].demand2 - m.z36_1[t] + m.u36_1[t])**2  # constraint 24 orders
        if t - m.delay >= m.T.first() and m.TimeStep + m.delay <= m.NumPeriods - 1:
        ...
        if t - m.delay5 >= m.T.first() and m.TimeStep + m.delay5 <= m.NumPeriods - 1:
            obj -= m.rho / 2 * (m.nb[t - m.delay5].s1 - m.z35_2[t] + m.u35_2[t]) ** 2  # constraint 23 goods
        if t - m.delay6 >= m.T.first() and m.TimeStep + m.delay6 <= m.NumPeriods - 1:
            obj -= m.rho / 2 * (m.nb[t - m.delay6].s2 - m.z36_2[t] + m.u36_2[t]) ** 2  # constraint 14 goods
...
def obj_rule_5(m):
    # Sum of Profit for all timeperiods
    return sum(m.nb[t].s * P4 - m.nb[t].r * C4 - m.nb[t].i * IC5 - m.nb[t].bl * BC5 -
               m.rho/2 * (m.nb[t].r - m.z35_1[t] + m.u35_1[t])**2
               if (t - m.delay < m.T.first() or m.TimeStep + m.delay > m.NumPeriods - 1) else
               m.nb[t].s * P4 - m.nb[t].r * C4 - m.nb[t].i * IC5 - m.nb[t].bl * BC5 -
               m.rho / 2 * (m.nb[t].r - m.z35_1[t] + m.u35_1[t]) ** 2 -
               m.rho / 2 * (m.nb[t].a - m.z35_2[t] + m.u35_2[t]) ** 2
               for t in m.T)

def obj_rule_6(m):
    # Sum of Profit for all timeperiods
    return sum(m.nb[t].s * P4 - m.nb[t].r * C4 - m.nb[t].i * IC6 - m.nb[t].bl * BC6 -
               m.rho/2 * (m.nb[t].r - m.z36_1[t] + m.u36_1[t])**2
               if (t - m.delay < m.T.first() or m.TimeStep + m.delay > m.NumPeriods - 1) else
               m.nb[t].s * P4 - m.nb[t].r * C4 - m.nb[t].i * IC6 - m.nb[t].bl * BC6 -
               m.rho / 2 * (m.nb[t].r - m.z36_1[t] + m.u36_1[t]) ** 2 -
               m.rho / 2 * (m.nb[t].a - m.z36_2[t] + m.u36_2[t]) ** 2
               for t in m.T)
...
for j in range(num_tests):
    ...
    for d in range(num_periods):
        ...
        if d - d5 < 0:
            ...
            if extra_delay:
                a50 = 0
                LP_shipment[d - d5 + 1, 4] += LP_shipment[d - d5, 4]
            else:
        ...
        if d - d6 < 0:
            ...
            if extra_delay:
                a60 = 0
                LP_shipment[d - d6 + 1, 5] += LP_shipment[d - d6, 5]
            else:
        ...
        model_2.B10 = pyo.Param(default=bl230)
        model_2.B20 = pyo.Param(default=bl240)
        ...
        model_2.bl23_linking = pyo.Constraint(model_2.T, rule=bl1_linking_rule)
        model_2.bl24_linking = pyo.Constraint(model_2.T, rule=bl2_linking_rule)
        ...
        model_3.B10 = pyo.Param(default=bl350)
        model_3.B20 = pyo.Param(default=bl360)
        model_3.delay = pyo.Param(default=d3)
        model_3.delay5 = pyo.Param(default=d5)
        model_3.delay6 = pyo.Param(default=d6)
        model_3.T = pyo.RangeSet(d, num_periods - 1)

        model_3.nb = pyo.Block(model_2.T, rule=node_div_block_rule)
        model_3.i_linking = pyo.Constraint(model_3.T, rule=i_linking_rule)
        model_3.bl35_linking = pyo.Constraint(model_3.T, rule=bl1_linking_rule)
        model_3.bl36_linking = pyo.Constraint(model_3.T, rule=bl2_linking_rule)
        ...
        Customer_Demand4 = {t: LP_Customer_Demand[j][0][t] for t in model_4.T}
        ...
        Customer_Demand5 = {t: LP_Customer_Demand[j][1][t] for t in model_5.T}
        model_5.Customer_Demand = pyo.Param(model_5.T, default=Customer_Demand5, mutable=False)
        ...
        model_5.d_linking = pyo.Constraint(model_5.T, rule=demand_linking_rule)
        ...
        Customer_Demand6 = {t: LP_Customer_Demand[j][2][t] for t in model_6.T}
        model_6.Customer_Demand = pyo.Param(model_6.T, default=Customer_Demand6, mutable=False)
        ...
        model_6.d_linking = pyo.Constraint(model_6.T, rule=demand_linking_rule)
        ...
        for i in range(N_iter):
            ...
            old_z35_1 = np.array([pyo.value(model_3.z35_1[t]) for t in model_3.T])
            old_z36_1 = np.array([pyo.value(model_3.z36_1[t]) for t in model_3.T])
            if d + d2 <= num_periods - 1:
            ...
            if d + d5 <= num_periods - 1:
                old_z35_2 = np.array([pyo.value(model_3.z35_2[t]) for t in model_3.T35])
            if d + d6 <= num_periods - 1:
                old_z36_2 = np.array([pyo.value(model_3.z36_2[t]) for t in model_3.T36])
            ...
            demand_23 = [pyo.value(model_2.nb[t].demand1) for t in model_2.T]
            z23_1_arr = (np.array(reorder_3) + np.array(demand_23)) / 2
            z23_1 = {t: z23_1_arr[i] for i, t in enumerate(model_3.T)}

            if d + d3 <= num_periods - 1:
                shipping_23 = [pyo.value(model_2.nb[t - d3].s1) for t in model_2.T23]
            ...
            demand_24 = [pyo.value(model_2.nb[t].demand2) for t in model_2.T]
            z24_1_arr = (np.array(reorder_4) + np.array(demand_24)) / 2
            z24_1 = {t: z24_1_arr[i] for i, t in enumerate(model_4.T)}

            if d + d4 <= num_periods - 1:
                shipping_24 = [pyo.value(model_2.nb[t - d4].s2) for t in model_2.T24]
            ...
            demand_35 = [pyo.value(model_3.nb[t].demand1) for t in model_3.T]
            z35_1_arr = (np.array(reorder_5) + np.array(demand_35)) / 2
            z35_1 = {t: z35_1_arr[i] for i, t in enumerate(model_5.T)}

            if d + d5 <= num_periods - 1:
                shipping_35 = [pyo.value(model_3.nb[t - d5].s1) for t in model_3.T35]
                acquisition_5 = [pyo.value(model_5.nb[t].a) for t in model_5.T35]
                z35_2_arr = (np.array(shipping_35) + np.array(acquisition_5)) / 2
                z35_2 = {t: z35_2_arr[i] for i, t in enumerate(model_5.T35)}

            # 36 Global variables
            reorder_6 = [pyo.value(model_6.nb[t].r) for t in model_6.T]
            demand_36 = [pyo.value(model_3.nb[t].demand2) for t in model_3.T]
            z36_1_arr = (np.array(reorder_6) + np.array(demand_36)) / 2
            z36_1 = {t: z36_1_arr[i] for i, t in enumerate(model_6.T)}

            if d + d6 <= num_periods - 1:
                shipping_36 = [pyo.value(model_3.nb[t - d6].s2) for t in model_3.T36]
                acquisition_6 = [pyo.value(model_6.nb[t].a) for t in model_6.T36]
                z36_2_arr = (np.array(shipping_36) + np.array(acquisition_6)) / 2
                z36_2 = {t: z36_2_arr[i] for i, t in enumerate(model_6.T36)}
            ...
            for t in model_1.T:
                ...
                model_3.z35_1[t] = z35_1[t]
                model_3.z36_1[t] = z36_1[t]
                model_4.z24_1[t] = z24_1[t]
                model_5.z35_1[t] = z35_1[t]
                model_6.z36_1[t] = z36_1[t]
            if d + d2 <= num_periods - 1:
            ...
            if d + d5 <= num_periods - 1:
                for t in model_3.T35:
                    model_3.z35_2[t] = z35_2[t]
                    model_5.z35_2[t] = z35_2[t]
            if d + d6 <= num_periods - 1:
                for t in model_3.T36:
                    model_3.z36_2[t] = z36_2[t]
                    model_6.z36_2[t] = z36_2[t]
            ...
            # 23
            u23_1_model2 = {t: pyo.value(model_2.u23_1[t]) + pyo.value(model_2.nb[t].demand1 - z23_1[t])
                            for t in model_2.T}
            u23_1_model3 = {t: pyo.value(model_3.u23_1[t]) + pyo.value(model_3.nb[t].r - z23_1[t])
                            for t in model_3.T}

            if d + d3 <= num_periods - 1:
                u23_2_model2 = {t: pyo.value(model_2.u23_2[t]) + pyo.value(model_2.nb[t - d3].s1 - z23_2[t])
                                for t in model_2.T23}
                u23_2_model3 = {t: pyo.value(model_3.u23_2[t]) + pyo.value(model_3.nb[t].a - z23_2[t])
                                for t in model_3.T23}

            # 24
            u24_1_model2 = {t: pyo.value(model_2.u24_1[t]) + pyo.value(model_2.nb[t].demand2 - z24_1[t])
                            for t in model_2.T}
            u24_1_model4 = {t: pyo.value(model_4.u24_1[t]) + pyo.value(model_4.nb[t].r - z24_1[t])
                            for t in model_4.T}

            if d + d4 <= num_periods - 1:
                u24_2_model2 = {t: pyo.value(model_2.u24_2[t]) + pyo.value(model_2.nb[t - d4].s2 - z24_2[t])
                                for t in model_2.T24}
                u24_2_model4 = {t: pyo.value(model_4.u24_2[t]) + pyo.value(model_4.nb[t].a - z24_2[t])
                                for t in model_4.T24}

            # 35
            u35_1_model3 = {t: pyo.value(model_3.u35_1[t]) + pyo.value(model_3.nb[t].demand1 - z35_1[t])
                            for t in model_3.T}
            u35_1_model5 = {t: pyo.value(model_5.u35_1[t]) + pyo.value(model_5.nb[t].r - z35_1[t])
                            for t in model_5.T}

            if d + d5 <= num_periods - 1:
                u35_2_model3 = {t: pyo.value(model_3.u35_2[t]) + pyo.value(model_3.nb[t - d5].s1 - z35_2[t])
                                for t in model_3.T35}
                u35_2_model5 = {t: pyo.value(model_5.u35_2[t]) + pyo.value(model_5.nb[t].a - z35_2[t])
                                for t in model_5.T35}

            # 36
            u36_1_model3 = {t: pyo.value(model_3.u36_1[t]) + pyo.value(model_3.nb[t].demand2 - z36_1[t])
                            for t in model_3.T}
            u36_1_model6 = {t: pyo.value(model_6.u36_1[t]) + pyo.value(model_6.nb[t].r - z36_1[t])
                            for t in model_6.T}

            if d + d6 <= num_periods - 1:
                u36_2_model3 = {t: pyo.value(model_3.u36_2[t]) + pyo.value(model_3.nb[t - d6].s2 - z36_2[t])
                                for t in model_3.T36}
                u36_2_model6 = {t: pyo.value(model_6.u36_2[t]) + pyo.value(model_6.nb[t].a - z36_2[t])
                                for t in model_6.T36}
            ...
            if ADMM:
                for t in model_1.T:
                    ...
                    model_3.u35_1[t] = u35_1_model3[t]
                    model_3.u36_1[t] = u36_1_model3[t]
                    model_4.u24_1[t] = u24_1_model4[t]
                    model_5.u35_1[t] = u35_1_model5[t]
                    model_6.u36_1[t] = u36_1_model6[t]
                if d + d2 <= num_periods - 1:
                    for t in model_1.T12:
                ...
                if d + d5 <= num_periods - 1:
                    for t in model_3.T35:
                        model_3.u35_2[t] = u35_2_model3[t]
                        model_5.u35_2[t] = u35_2_model5[t]
                if d + d6 <= num_periods - 1:
                    for t in model_3.T36:
                        model_3.u36_2[t] = u36_2_model3[t]
                        model_6.u36_2[t] = u36_2_model6[t]

            # residual calculation
            old_z = np.concatenate((old_z12_1, old_z23_1, old_z24_1, old_z35_1, old_z36_1))
            z = np.concatenate((z12_1_arr, z23_1_arr, z24_1_arr, z35_1_arr, z36_1_arr))
            ...
            prim_r_z35_1 = [
                np.linalg.norm([np.array(reorder_5)[i] - z35_1_arr[i], np.array(demand_35)[i] - z35_1_arr[i]]) for i in
                range(len(z35_1_arr))]
            prim_r_z.append(prim_r_z35_1)

            prim_r_z36_1 = [
                np.linalg.norm([np.array(reorder_6)[i] - z36_1_arr[i], np.array(demand_36)[i] - z36_1_arr[i]]) for i in
                range(len(z36_1_arr))]
            prim_r_z.append(prim_r_z36_1)
            ...
            if d + d5 <= num_periods - 1:
                old_z = np.concatenate((old_z, old_z35_2))
                z = np.concatenate((z, z35_2_arr))
                prim_r_z35_2 = [
                    np.linalg.norm([np.array(acquisition_5)[i] - z35_2_arr[i], np.array(shipping_35)[i] - z35_2_arr[i]])
                    for i in range(len(z35_2_arr))]
                prim_r_z.append(prim_r_z35_2)

            if d + d6 <= num_periods - 1:
                old_z = np.concatenate((old_z, old_z36_2))
                z = np.concatenate((z, z36_2_arr))
                prim_r_z36_2 = [
                    np.linalg.norm([np.array(acquisition_6)[i] - z36_2_arr[i], np.array(shipping_36)[i] - z36_2_arr[i]])
                    for i in range(len(z36_2_arr))]
                prim_r_z.append(prim_r_z36_2)
        ...
        bl230 = pyo.value(model_2.nb[d].bl1)
        bl240 = pyo.value(model_2.nb[d].bl2)
        bl30 = pyo.value(model_3.nb[d].bl)
        bl350 = pyo.value(model_3.nb[d].bl1)
        bl360 = pyo.value(model_3.nb[d].bl2)
        ...
        LP_shipment[d, :] = [pyo.value(model_1.nb[d].r), pyo.value(model_1.nb[d].s),
                             pyo.value(model_2.nb[d].s1), pyo.value(model_2.nb[d].s2),
                             pyo.value(model_3.nb[d].s1), pyo.value(model_3.nb[d].s2)]
